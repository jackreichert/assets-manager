...
class wp_assets_manager {
	...
	public function __construct() {
		...
		add_action( 'add_meta_boxes', array(
			$this,
			'assets_manager_register_meta_box'
		) ); # creates meta for uploading, managing assets
		add_action( 'admin_enqueue_scripts', array( $this, 'load_admin_scripts' ) ); # load admin js
		add_action( 'wp_ajax_update_asset', array(
			$this,
			'update_asset_action_func'
		) ); # plupload ajax function_exists
				...
				<?php foreach ( $attachments as $i => $asset ) :
					...
					<li id="<?php echo $asset->ID; ?>" class="asset">
						...
						<div class="assetMeta">
							...
							<i<?php echo( ( $this->has_expired( $asset->ID ) ) ? ' class="expired"' : '' ); ?>>(<?php echo $this->get_expiry_date( $asset->ID ); ?>
								)</i><br>
	...
	public function load_admin_scripts() {
		...
		if ( is_admin() && is_object( $post ) && 'asset' === $post->post_type ) {
			...
			wp_enqueue_script( 'wp_assets', plugin_dir_url( __FILE__ ) . '/js/wp-assets-manager.js', array('jquery','plupload-all'), 201510 );
	...
	public function update_asset_action_func() {
		...
		$response = array(
			'post_vals'  => array(
				...
				'url'         => $this->get_asset_link( $current_asset->ID ),
